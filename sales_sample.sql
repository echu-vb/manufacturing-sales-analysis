...
-- Sales by year where products were shipped
SELECT YEAR_ID AS SALESYEAR, ROUND(SUM(SALES),3) AS TOTAL_SALES
FROM sample_dataset.sales_data_sample
WHERE STATUS = 'Shipped'
GROUP BY SALESYEAR
...
-- KPI
SELECT ROUND(SUM(SALES),3) AS TOTAL_SALES, COUNT(DISTINCT CUSTOMERNAME) AS '# of Customers'
FROM sample_dataset.sales_data_sample
WHERE STATUS = 'Shipped';

-- Sales by Month for 2005
SELECT MONTH_ID, ROUND(SUM(SALES),3) AS TOTAL_SALES
FROM sample_dataset.sales_data_sample
WHERE STATUS = 'Shipped'
AND YEAR_ID = 2005
...
-- What happened in 2005 that causes the sudden decrease in sales?
SELECT PRODUCTLINE, ROUND(SUM(SALES),3) AS '2004 SALES'
FROM sample_dataset.sales_data_sample
WHERE YEAR_ID = 2004
AND STATUS = 'Shipped'
GROUP BY PRODUCTLINE
...
SELECT PRODUCTLINE, ROUND(SUM(SALES),3) AS '2005 SALES'
FROM sample_dataset.sales_data_sample
WHERE YEAR_ID = 2005
AND STATUS = 'Shipped'
GROUP BY PRODUCTLINE
